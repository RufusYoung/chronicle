...
凡是进入生产、运输、建造、生存或战斗循环的物资，不得只作为商店中的可购买数字。实现必须至少说明并验证：
...
使用如何改变成本、风险或结果
何时消耗、磨损、损坏或遗失
形成缺口后谁会补充、等待、改用替代品或放弃
```

默认模拟单位是“物品批次或有意义的单件 × 作业周期”。除非微观差异会改变选择或结果，不追踪纤维、绳结、每次挥动等细节。可见文本可以压缩链条，但 Fact、Obligation、Item、Exchange、State 等真值必须能还原链条。

新增物资系统优先使用配置化的通用作业材料规则，不为绳索、木料、药品或攀登器材各复制一套流程。首个物资样本通过，只证明合同可行；至少完成不同用途和反事实验证后，才可声称形成通用经济颗粒度。
