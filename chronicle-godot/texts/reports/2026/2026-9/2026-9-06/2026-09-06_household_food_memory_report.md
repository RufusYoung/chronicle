...
- 完整常规回归含真实渲染 134/134 通过；不包含可选的三十日长期测试。源码和实际 Windows 包的进程级后台协议均 9/9 通过，新增一组只用公开世界观察接口推进两日、分页读取事实，并确认自主交付进入真实进食的检查。
...
- 运行实现提交 `3210adcffec4141297e42f6563be448b5f261cf2` 已推送，并从干净工作区重新导出 Windows 包。该包的 9/9 后台协议、新世界实际绘制和旧 H1 七日存档启动均通过。新进程到可控绘制帧各测一次为 2.56/2.72 秒；后一个是旧合成世界存档，不能写成新家庭世界的第七日启动数据。构建清单、接口日志与启动证据已归档。随后文档/测试记录提交若再次刷新包，以本地 `builds/h1-windows/build_manifest.json` 及相邻验证文件为准，不把文档提交误称为新机制版本。
