...
func execute_trade(
	...
	if buyer_entity_id == "" or seller_entity_id == "" or quantity < 1:
	...
	transaction.add_fact({
		"fact_id": fact_id,
		"fact_type": "market_trade_completed",
	...
	return {
		...
		"transaction_result": transaction.to_dict(),
	}
...
func _find_payment_stack(
	...
	for item: Dictionary in item_store.list_items_for_owner(owner_id):
		if (
			...
			and int(item.get("quantity", 0)) >= quantity
		):
...
func _add_stack_transfer(
	...
	if quantity == int(item.get("available_quantity", item.get("quantity", 0))):
