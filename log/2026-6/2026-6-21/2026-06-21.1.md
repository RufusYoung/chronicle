# 2026-06-21.0 指令：整理 Chronicle v5 文档索引

## 任务目标

本次只做第 0 步：整理 Chronicle v5 文档入口。
...
请新增一个文档索引文件，用于说明这三个文件的定位、阅读顺序、后续文档扩展规则和开发前置约束。

本次不要改 Godot 项目，不要改游戏脚本，不要改场景，不要迁移旧代码。
...
如果已有 `v5/README.md`，则在不破坏原内容的基础上更新。
...
```text
v5/CHRONICLE_REBUILD_ROADMAP_v5.1.md
```
...
```text
v5/CHRONICLE_LOCATION_UI_FLOW_v5.1.md
```
...
```bash
git push -u origin docs/v5-document-index
```
